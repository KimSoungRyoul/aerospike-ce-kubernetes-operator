...
| `maxUnavailable` | [IntOrString](https://pkg.go.dev/k8s.io/apimachinery/pkg/util/intstr#IntOrString) | 아니요 | 이 rack에서 voluntary disruption이 허용되는 최대 pod 수. 이 rack의 PDB에 대해 `spec.maxUnavailable`를 override 합니다. 미설정 시 `replication-factor - 1` (이 rack의 effective `aerospikeConfig` 기준). 클러스터 크기가 아니라 이 rack의 pod 수를 기준으로 검증됩니다. |
