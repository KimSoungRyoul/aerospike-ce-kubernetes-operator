...
**Use case:** Production with Prometheus/Grafana observability. The operator injects an exporter sidecar into each pod, creates a ServiceMonitor for automatic discovery, and generates a PrometheusRule with built-in alerts (NodeDown, StopWrites, HighDiskUsage, HighMemoryUsage). You can replace the built-in alerts with custom rules using `prometheusRule.customRules`. See the [Monitoring guide](../operations/monitoring.md) for details.
